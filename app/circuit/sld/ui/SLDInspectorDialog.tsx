                                    ...
                                    {isNode(selectedElement) && currentElementNodeSLDType === SLDElementType.Grid && (
                                        <ConfigCard title="Grid Connection Configuration" icon={Building}>
                                            <GridSection cols={2}>
                                                ...
                                                <div className="space-y-1"> <Label htmlFor="config.frequencyHz" className="text-xs">Frequency (Hz)</Label> <Select value={String((formData.config as GridNodeData['config'])?.frequencyHz || '')} onValueChange={(val) => handleSelectChange("config.frequencyHz", val ? parseInt(val) : undefined)}> <SelectTrigger className="h-8 text-xs"><SelectValue placeholder="Select frequency..." /></SelectTrigger> <SelectContent><SelectItem value="" className="text-xs text-muted-foreground italic">Not Specified</SelectItem><SelectItem value="50" className="text-xs">50 Hz</SelectItem><SelectItem value="60" className="text-xs">60 Hz</SelectItem></SelectContent> </Select> </div>
